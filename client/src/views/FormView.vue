...
import { NButton, NDivider, useMessage, NResult } from "naive-ui";
...
<template>
  <main>
    ...
    <div v-if="status?.status === 'error'">
      <n-divider />
      <n-result status="error" title="Error" :description="status.error" />
    </div>
    <div v-else-if="status?.status === 'session-nonexistent'">
      <p>
